...
while q<nq && (abs(K1-K0)>tolk || abs(L1-L0)>tollab)
    ...
    % Prices
    r0 = alpha*((L0/K0)^(1-alpha))-delta; % <------------------- YOUR CODE GOES HERE
    w0 = (1-alpha)*((K0/L0)^alpha); % <------------------- YOUR CODE GOES HERE

    % Pension benefit
    b = (tau*w0*L0) / sum(mass(JR:J)); % <------------------- YOUR CODE GOES HERE
    ...
    for j=tW:-1:1           % age
        for ik0=1:nk          % assets today
            ...
            while ik1 < nk  	% assets tomorrow
                ...
                lab = gamma - ((1-gamma)*(((1+r0)*kap0)-kap1)/((1-tau)*w0*e(j))); % <------------------- YOUR CODE GOES HERE !!
