...
\begin{enumerate}[(a)]
    ...
    \item %First, note that all inequalities hold for all$\theta\geq0$.Then, suppose $\theta\leq0$. If $\theta\in[-1, 0]$, all inequalities hold. :\begin{align*}
     %   g(0, 1) \geq g(0, 0) &\Rightarrow g(1, 1) \geq g(0, 0)  \\
     %   \therefore \theta &\geq -1                              
    %\end{align*}
    
    %If $\theta=1$, then $\theta$ is no longer identified by inequality (5). Since (5) is the only nontrivial inequality that compares $(0, 1)$ and $(1, 0)$, This model cannot identify the relative ordering of these two outcomes.
    
    
    %then the set of inequalities becomes:\begin{align*}
    %    g(1, 0) &\geq g(0, 0)   \\
    %    g(1, 1) &\geq g(0, 0)   \\
    %    g(0, 1) &\geq g(0, 0)   \\
    %    g(1, 1) &\geq g(1, 0)   \\
